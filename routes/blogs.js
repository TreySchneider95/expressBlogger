...
router.delete('/single/:blogTitleToDelete', (req, res)=>{
    const blog = blogs.findIndex((blog)=>{
...
router.put('/update-one/:title', (req, res)=>{
    const blogToUpdate = blogs.findIndex((blog)=>{
        return blog.title === req.params.title
    })
    const blogValidResponse = blogValidate[1](req.body, blogs[blogToUpdate])
    if (blogValidResponse.success){
        blogValidResponse.updatedBlog.lastModified = new Date()
        blogs[blogToUpdate] = blogValidResponse.updatedBlog
        res.json({
            success: true
        })
    }
